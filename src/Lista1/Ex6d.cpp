...
int main()
{
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Ola Triangulo! -- Rodrigo", nullptr, nullptr);
	if (!window)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Exercicio 6A\tFPS %.2lf", fps);
		...
		glUniform4f(colorLoc, 0.0f, 2.0f, 1.0f, 1.0f); // enviando cor para variável uniform inputColor
		glDrawArrays(GL_LINE_LOOP, 0, 3);
        glDrawArrays(GL_LINE_LOOP, 3, 3);

		glUniform4f(colorLoc, 0.0f, 4.0f, 2.0f, 4.0f); // enviando cor para variável uniform inputColor
